    ...
    for k in keys:
        if k not in spec:
            raise ValueError("Key %s not found in dataset.  Available keys are %s"
                             % (k, spec.keys()))
        if not spec[k].shape.is_compatible_with(tf.TensorShape([None])):
...
class SequenceDatasetPacker(object):
    ...
    def _finalize(self, _, contents):
        ...
        check_leaks = tf.debugging.assert_none_equal(contents, -tf.ones_like(contents))
...
def _scan_step_fn(state, example, packed_length, queue_size, spacing,
    ...
    if any_can_fit:
        ...
        shifted_range = (tf.range(queue_size, dtype=INDEX_DTYPE) - top_index) % queue_size
