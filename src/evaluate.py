...
def get_dataset(dataset_path: Path, batch_size: int, packed_seq_len: int, take: int = None, shuffle_buffer: int = 1000):

    def parse_json(json_string_tensor):
        return tf.constant(json.loads(json_string_tensor.numpy())["encoded"], dtype=tf.int32)
...
def evaluate(vocab_path: Path, checkpoint_path: Path, dataset_path: Path, batch_size: int, packed_seq_len: int,
             take: int = None, shuffle_buffer: int = None):
    # Vocab
    ...
    # Dataset
    ds = get_dataset(str(dataset_path), batch_size=batch_size, packed_seq_len=packed_seq_len,
                     take=take, shuffle_buffer=shuffle_buffer)
    ...
    @tf.function
    def evaluation_step(batch_text, batch_pos, batch_seg):
        ...
        logits, _ = transformer_decoder(model_input, model_input_pos, False, attention_mask)
        ...
        # Update metrics
        token_accuracy(ground_truth, logits, sample_weight=mask)
        log_ppl(tf.nn.sparse_softmax_cross_entropy_with_logits(ground_truth, logits) / tf.math.log(2.0),
                sample_weight=mask)

    for batch in ds:
        evaluation_step(batch["text"], batch["text_position"], batch["text_segmentation"])

    # Decode some examples
    gt_examples = []
    random_sampling_examples = []
    top_5_sampling_examples = []
    for example in get_dataset(str(dataset_path), batch_size=1, packed_seq_len=packed_seq_len, take=None)\
            .shuffle(1000, seed=42).take(5):
        # Use the first 4 tokens as seed
        gt_examples.append(
            vocab.decode(tf.boolean_mask(example["text"][0], tf.equal(example["text_segmentation"][0], 1)).numpy())
        )
        random_sampling_examples.append(
            vocab.decode(decode_encoded(example["text"][0][:4].numpy(), transformer_decoder, vocab.end_idx,
    ...
    try:
        while True:
            ...
            res = evaluate(Path(flags.FLAGS.vocab), checkpoint_path, Path(flags.FLAGS.data),
                     flags.FLAGS.batch_size, flags.FLAGS.packed_seq_len, flags.FLAGS.take, flags.FLAGS.shuffle_buffer)
...
if __name__ == '__main__':
    ...
    flags.DEFINE_integer("packed_seq_len", 600, help="Packed sequence length")
